...
int main() {
    cout << "Paste the LALR parse table (PRODUCTIONS, ACTION, GOTO) and press Enter twice:\n";
    read_parser_input();
    ...
    while (ip < tokens.size()) {
        ...
        vector<string> stack_parts;
        while (!temp_stack.empty()) {
            stack_parts.push_back(to_string(temp_stack.top().second));
            stack_parts.push_back(temp_stack.top().first);
            temp_stack.pop();
        }
        reverse(stack_parts.begin(), stack_parts.end());
        for(const auto& part : stack_parts) stack_ss << part << " ";
        printf("%-35s | ", stack_ss.str().c_str());

        stringstream input_ss;
        for (int i = ip; i < tokens.size(); ++i) input_ss << tokens[i] << " ";
        printf("%-25s | ", input_ss.str().c_str());

        if (action_table.size() <= current_state || action_table[current_state].find(current_symbol) == action_table[current_state].end() || action_table[current_state][current_symbol].empty()) {
        ...
        if (action[0] == 's') {
            ...
            if (goto_table.size() <= state_after_pop || goto_table[state_after_pop].find(lhs) == goto_table[state_after_pop].end()){
                cout << "Error: Invalid GOTO\n";
                error = true;
                break;
            }
